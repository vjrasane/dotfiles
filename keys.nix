let
  ...
  master = {
    privateKeyPath = homeDir: "${homeDir}/.ssh/id_rsa";
    publicKeyPath = homeDir: "${master.privateKeyPath homeDir}.pub";
    publicKey = "ssh-rsa ";
  };

  currentMachine = {
    privateKeyPath = homeDir: "${homeDir}/.ssh/id_ed25519";
    publicKeyPath = homeDir: "${currentMachine.privateKeyPath homeDir}.pub";
    publicKey = homeDir: builtins.replaceStrings ["\n"] [""] (builtins.readFile ( currentMachine.publicKeyPath homeDir ));
  };
in {
  inherit age master currentMachine;
  encryptionKeys = [age.publicKey master.publicKey thinkpad];
  signingKeys = homeDir: [master.publicKey (currentMachine.publicKey homeDir)];
}
